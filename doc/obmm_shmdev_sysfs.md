...
通过 sysfs 目录下有多个只读文本文件和只读二进制文件。每个文件展示了该内存设备的一项属性。可用于维测与调试，不建议用于控制面功能、和性能路径。
...
**priv**
类型：二进制文件
描述：内存私有元数据，内容由用户创建内存设备时传入，文件大小为`OBMM_MAX_PRIV_LEN`，其中超过偏移量超过`priv_len`的部分仍可读取，但其二进制值恒为0。
...
类型：文本、十进制数
描述：`0` 代表导出内存的由 OBMM 分配，`1` 代表导出内存来自导出进程。
